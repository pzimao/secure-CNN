...
ConvLayer conv_2d_ptU(const ConvLayer& in, Filter2D& filter, bool same, const ui32 p){
                ...
                for(ui32 m=0; m<filter.shape.in_chn; m++){
                    for (ui32 f_h = 0; f_h < filter.shape.f_h; f_h++){
                        for (ui32 f_w = 0; f_w < filter.shape.f_w; f_w++){
                            ...
                            // 如果这个数是负数，则用带符号数处理
                            if(filter.w[n][m][f_h][f_w]>p/2){
                                temp += ((filter.w[n][m][f_h][f_w]-p)*in_act);
                            }else{
                                temp += ((filter.w[n][m][f_h][f_w]*in_act));
                            }                         
                        }
                    }
                }
                out.act[n][h][w] = ((temp/100)+p)%p;
            }        
        }
...
vector<vector<vector<vector<uv64>>>>readInputWeightGroup(string path,int in_chn,const FVParams & params){
    
    uv64 temp_line;
    vector<uv64> temp_matrx;
    vector<vector<uv64>> temp_weight;
    vector<vector<vector<uv64>>> weight;
    vector<vector<vector<vector<uv64>>>> group_weight;
    string line;
    ifstream in(path);
    regex pat_regex("[-]?([0-9]+.[0-9]+)");
    while(getline(in, line)) { 
            ...
            if(temp_line.size() > 0){
                ...
                temp_weight.push_back(temp_matrx);
                temp_matrx.clear();
                if(temp_weight.size()==8){
                    weight.push_back(temp_weight); 
                    temp_weight.clear(); 
                } 
                if(weight.size()==8){
                    group_weight.push_back(weight);
                    weight.clear();
                }
            }              
        }
...
CTMat preprocess_recover_ifmap(CTMat &ifmap, const uv64 &r, const ConvShape& shape,
        const ui32 window_size, const ui32 num_windows, const FVParams & params)
{
...
CTMat preprocess_recover_ifmapU(CTMat &ifmap, const uv64 &vec_r, const ConvShape & shape,
        const ui32 window_size, const ui32 num_windows, const FVParams & params)
{
    ui32 chn_pow2 = nxt_pow2(shape.h*shape.w);
    ui32 row_pow2 = nxt_pow2(shape.w);
...
vector<vector<vector<uv64>>> trans_to_matrix_group(ui32 goup_num,vector<uv64> vec, ui32 in_h,ui32 in_w){
...
CTVec comput_convolution(int conv_type,const SecretKey& sk,uv64 r,const ConvLayer& ifmap, Filter2D& filter,const FVParams& params){
    double start, stop;
    ...
    //------------------------server:加密的结果需要减去r-----------------  
    ct_mat = preprocess_recover_ifmap(ct_mat, r, ifmap.shape, pt_window_size, pt_num_windows, params);
        ...
        for(int j=0;j<act[i].size();j++){
            for(int k=0;k<act[i][j].size();k++){
                 if(act[i][j][k]<params.p/2){
                    ...
                    ui64 a=act[i][j][k];
                    value=((params.p-act[i][j][k])/10.0)*(-1);
                }
                temp.push_back(value);
            }
            cheack<<vec_to_str(temp)<<endl;
...
int  main() {
    ...
    //cout<<"------------------------------------执行conv2d_2------------------------------"<<endl;
    //第二层的卷积，应该是从文件里读入密文，也就是读入maxpool的结果（密文）
    string input_path="../result/maxpool_4_out.txt";
    string path_parameter="../params/con5_1_param.txt";
    string path_output="../result/5_1_output.txt";
    string path_outkey="../params/outkey.txt";
    string conv_filter="../params/5_1w.txt";
    string path_bias="../params/5_1b.txt";
    //cout<<"----------------------从文件中读入参数------------------------"<<endl;
    vector<ui64> v_paramater=read_paramater(path_parameter);
    //cout<<"-------------------------从文件中读取参数并打印------------------------"<<endl;
    ...
    vector<uv64> x_r;//客户端拥有
    uv64 r=input[0][0];//第一个是噪声
    for(int i=1;i<input[0].size();i++){
        x_r.push_back(input[0][i]);
    }

    //用作测试
    vector<uv64> vec_x;
    uv64 x(test_params.phim);
    uv64 r_negative=get_nagative(r,test_params);
    //-----------------------将噪声r减去----------------------------------
    vec_x=eliminate_noise(x_r,r_negative,test_params);
    input.clear();
    input.shrink_to_fit();
    ...
    x_r_matrix=trans_to_matrix_group(map_num_per_group,x_r,in_h,in_w);
    ...
    for(ui32 round=0;round<out_chn/chn_per;round++){
        ...
        for(ui32 ngroup=0;ngroup<group_num;ngroup++){
            ...
            //给滤波器赋值
            //filter.w=filter_group[round][ngroup];     
            filter.w=w[ngroup];     
            auto ofmap_ref = conv_2d_pt(ifmap_pt, filter, true, opt::p);
            auto ct=comput_convolution(conv_type,sk,r,ifmap,filter,test_params);
        ...
        //---------------------------在密文上加入噪声并交给客户端解密---------------------------------
        uv64 noise(test_params.phim);
        for(int i=0;i<test_params.phim;i++){
            noise[i]=rand()%100;
        }
        CTVec temp_ct;
        uv64 ct_r = empty_encrypt(noise, test_params);
        ...
        for(int i=0;i<noise.size();i++){
            noise[i]=noise[i]*group_num;
        }
        for(int i=0;i<ct_vec_sum.size();i++){
            ct_vec_sum[i]=EvalSubPlainU(ct_vec_sum[i],noise,test_params);
        }
        //64*64*3*3滤波器转变为32×64×3×3
        //明文计算
        auto ofmap_ref_x = conv_2d_pt(ifmap_x, filter_totol, true, opt::p);

        ui32 chn_pow2 = nxt_pow2(ifmap_x.shape.h*ifmap_x.shape.w);
